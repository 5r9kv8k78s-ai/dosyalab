...
// same timers, same polling pattern) for the DOCX → PDF direction. Kept as
// a separate component rather than sharing code with PdfToWordCard so the
// existing PDF → Word path isn't touched.

const MAX_FILE_SIZE_BYTES = 100 * 1024 * 1024;
const POLL_INTERVAL_MS = 800;

// The backend has no real per-page progress signal for the conversion
// itself (see apps/api/app/modules/converter/docx_to_pdf.py) — only upload
...
export function WordToPdfCard() {
    ...
    (file: File) => {
      if (!file.name.toLowerCase().endsWith('.docx')) {
        setState({
          ...initialState,
          stage: 'error',
          errorMessage: 'Only DOCX files are supported for this conversion.',
        });
        return;
      }
      if (file.size > MAX_FILE_SIZE_BYTES) {
        setState({
          ...initialState,
          stage: 'error',
          errorMessage: 'That file is larger than the 100MB limit for Word → PDF.',
        });
        return;
      }
      ...
      submitDocxToPdfConversion(file, (percent) => {
        setState((prev) =>
  ...
  return (
    <Card>
      <CardHeader>
        <FileTypeIcon type="word" size={40} />
        <div>
          <CardTitle as="h2">Word → PDF</CardTitle>
          <CardDescription>Convert a .docx file into a shareable PDF</CardDescription>
        </div>
      </CardHeader>
      ...
      <Dropzone
        ...
        disabled={isBusy}
        onFiles={handleFiles}
        aria-label="Drop a DOCX file here, or click to browse, to convert it to PDF"
      >
        {state.stage === 'idle' && (
          <>
