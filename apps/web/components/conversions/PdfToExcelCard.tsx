...
// same timers, same polling pattern) for the PDF → Excel direction. Kept as
// a separate component rather than sharing code with the other cards so the
// existing PDF → Word and Word → PDF paths aren't touched.

const MAX_FILE_SIZE_BYTES = 100 * 1024 * 1024;
const POLL_INTERVAL_MS = 800;

// The backend has no real per-page progress signal for the conversion
// itself (see apps/api/app/modules/converter/pdf_to_xlsx.py) — only upload
// has a true byte-level percentage. So conversion is represented as named
// stages with a bar that animates smoothly and continuously, never as a
// faked precise number.
type Stage =
  | 'idle'
  | 'uploading'
  | 'processing'
...
// How long to sit on "Processing" before the label advances to "Extracting
// tables". The backend doesn't expose a distinct signal for this hand-off,
// so it's a fixed narrative delay rather than a derived number — if the
// real job finishes first, the status poll short-circuits straight to
    ...
    (file: File) => {
      ...
      if (file.size > MAX_FILE_SIZE_BYTES) {
        setState({
          ...initialState,
          stage: 'error',
          errorMessage: 'That file is larger than the 100MB limit for PDF → Excel.',
        });
        return;
      }
      ...
      submitPdfToXlsxConversion(file, (percent) => {
        setState((prev) =>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <div>
          <CardTitle as="h2">PDF → Excel</CardTitle>
          <CardDescription>Extract PDF tables into a spreadsheet</CardDescription>
        </div>
      </CardHeader>
      ...
      <Dropzone
        ...
        disabled={isBusy}
        onFiles={handleFiles}
        aria-label="Drop a PDF here, or click to browse, to convert it to Excel"
      >
        {state.stage === 'idle' && (
          <>
            ...
            <p className="text-small text-muted mt-1">Up to 100MB</p>
          </>
        )}
