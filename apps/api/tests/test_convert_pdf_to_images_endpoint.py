...
def _upload(client: TestClient, content: bytes, **extra):
    return client.post(
...
def test_pdf_to_images_default_png_success(
    ...
    assert status_body["filename"] == "sample_pages.zip"
...
def test_pdf_to_images_custom_format_and_dpi(
    client_with_tmp_storage: TestClient, sample_pdf_bytes: bytes
) -> None:
    response = _upload(client_with_tmp_storage, sample_pdf_bytes, image_format="jpg", dpi="72")
    assert response.status_code == 202
    job_id = response.json()["job_id"]
    ...
    download_response = client_with_tmp_storage.get(status_body["download_url"])
    archive = zipfile.ZipFile(io.BytesIO(download_response.content))
    assert all(name.endswith(".jpg") for name in archive.namelist())


def test_pdf_to_images_rejects_unsupported_format(
    client_with_tmp_storage: TestClient, sample_pdf_bytes: bytes
) -> None:
    response = _upload(client_with_tmp_storage, sample_pdf_bytes, image_format="bmp")
    assert response.status_code == 400


def test_pdf_to_images_rejects_zero_dpi(
    client_with_tmp_storage: TestClient, sample_pdf_bytes: bytes
) -> None:
    response = _upload(client_with_tmp_storage, sample_pdf_bytes, dpi="0")
    assert response.status_code == 400


def test_pdf_to_images_rejects_corrupted_pdf(
...
def test_pdf_to_images_rejects_encrypted_pdf(
...
def test_pdf_to_images_rejects_oversized_file(sample_pdf_bytes: bytes, tmp_path) -> None:
